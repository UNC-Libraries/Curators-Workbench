...
public class CrosswalkBaseItemSemanticEditPolicy extends SemanticEditPolicy {
		...
		if (request instanceof ReconnectRequest) {
			...
			if (view instanceof View) {
				Integer id = new Integer(
						CrosswalkVisualIDRegistry.getVisualID((View) view));
	...
	private IElementType getContextElementType(IEditCommandRequest request) {
		IElementType requestContextElementType = CrosswalkElementTypes
	...
	public static LinkConstraints getLinkConstraints() {
		LinkConstraints cached = CrosswalkDiagramEditorPlugin.getInstance()
				.getLinkConstraints();
		if (cached == null) {
			CrosswalkDiagramEditorPlugin.getInstance().setLinkConstraints(
		...
		public boolean canExistInputOutput_4001(Input source, Output target) {
			...
			try {
				if (target == null) {
					...
					if (source.getInputType().isAssignableFrom(
							target.getOutputType())) {
						result = true;
					}
				}
			} catch (Exception e) {
