...
public class ArrangementCommonDropAdapterAssistant extends CommonDropAdapterAssistant {
	...
	if (LocalSelectionTransfer.getTransfer().isSupportedType(aTransferType)) {
	   ...
	   * else if
	   * (ResourceTransfer.getInstance().isSupportedType(aTransferType)) {
	   * LOG.debug("found resource transfer type"); return true; } else if
	   * (PluginTransfer.getInstance().isSupportedType(aTransferType)) {
	   * LOG.debug("found plugin transfer type"); return true; } /*else if
	...
	/*
	 * if (ResourceTransfer.getInstance().isSupportedType(aDropAdapter.
	 * getCurrentTransfer())) {
	 * LOG.debug("found resource transfer, event data: " + event.data);
	 * IResource[] res = (IResource[]) event.data; List<IResource> items =
	 * new ArrayList<IResource>(res.length); for (IResource r : res) {
	 * items.add(r); } return dropResources(items); } else
	 */
